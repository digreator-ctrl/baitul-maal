...
export default function EditDonaturPage() {
  const { id } = useParams();
  const { user } = useAuth();
  const { donatur, updateDonatur } = useData();
  ...
  const handleProvinsiChange = (e) => {
    ...
    setForm(prev => ({
      ...
      kotaId: '', kota: '', kecamatanId: '', kecamatan: '', kelurahanId: '', kelurahan: ''
    }));
  };
  ...
  const handleKotaChange = (e) => {
    ...
    setForm(prev => ({
      ...
      kota: selected?.name || '',
      kecamatanId: '', kecamatan: '', kelurahanId: '', kelurahan: ''
    }));
  };
      ...
      <form onSubmit={handleSubmit} noValidate>
        ...
        {step === 2 && (
          <div className="card animate-fade-in">
            ...
            {!form.provinsiId && d.provinsi && (
              <div className="mb-md" style={{ padding: '12px', background: 'var(--warning-bg, #fff8e1)', color: 'var(--warning-text, #b8860b)', borderRadius: 'var(--radius-md)', fontSize: '0.875rem' }}>
                <strong>Catatan:</strong> Data wilayah sebelumnya ({d.kelurahan}, {d.kecamatan}, {d.kota}, {d.provinsi}) menggunakan format teks lama. Silakan pilih ulang wilayah dari daftar dropdown di bawah ini.
              </div>
            )}
